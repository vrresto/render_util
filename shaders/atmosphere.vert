...
vec4 calcAtmosphereColor(vec3 obj_pos, float air_dist, float haze_dist, vec3 viewDir,
...
//   rayleighColor = mix(rayleighColor, diffuseScatteringColorBright, 1.0 - exp(-3 * d * 2.0));
  ...
  if (!is_sky)
  {
    ...
    float dist_to_object = length(cameraPosWorld - obj_pos);
...
void getFogParameters(vec3 obj_pos, vec3 obj_pos_flat, out vec4 atmosphere_color, out vec4 fog_color)
{
  debugColor = vec3(0);
...
//       intersectsGround(vec3(0, 0, cameraPosWorld.z + planet_radius), viewDir)
    )
  {
    ...
    {
      t = getMaxAtmosphereThicknessFromObjectReverse(obj_pos);
    }
    else
    {
      vec2 t1 = getMaxAtmosphereThicknessFromObjectReverse(obj_pos);
      vec2 t2 = getMaxAtmosphereThicknessFromCameraReverse(obj_pos);
      t = t1 - t2;
...
//       t = t2;
    ...
    vec2 t1 = getMaxAtmosphereThicknessFromCamera(obj_pos);
    vec2 t2 = getMaxAtmosphereThicknessFromObject(obj_pos);
    t = t1 - t2;
  }
  ...
  float fog_dist = 0;
  fog_dist += calcHazeDistance(obj_pos, obj_pos_flat);
  fog_dist += t.y;

  vec3 mie_color = vec3(0);

  atmosphere_color = calcAtmosphereColor(obj_pos,
    t.x, fog_dist, viewDir, fog_color.xyz, mie_color, false);
